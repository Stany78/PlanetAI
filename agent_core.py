...
# Inizializza geolocator Google (API key da secrets Streamlit)
try:
    import streamlit as st
    GOOGLE_API_KEY = st.secrets.get("GOOGLE_GEOCODING_API_KEY")
    if not GOOGLE_API_KEY:
        print("[GEO][WARN] GOOGLE_GEOCODING_API_KEY non trovata!")
        GOOGLE_API_KEY = None
except:
    GOOGLE_API_KEY = None

_geolocator = GoogleV3(api_key=GOOGLE_API_KEY) if GOOGLE_API_KEY else None


def geocode_indirizzo(comune: str, indirizzo: str) -> tuple[float, float, dict]:
    """
    Geocoda usando Google Geocoding API.
    Returns: (lat, lon, info_dict)
    """
    if not _geolocator:
        return (0, 0, {
            'success': False,
            'type': 'failed',
            'display_name': '',
            'message': '❌ Google API key non configurata'
        })
    
    full_address = f"{indirizzo}, {comune}, Italia"
    if DEBUG_MODE:
        print(f"[GEO] Google Geocoding: {full_address}")
    ...
    try:
        ...
        if loc is None:
            return (0, 0, {
                ...
                'type': 'failed',
                'display_name': '',
                'message': f'❌ Non trovato: "{indirizzo}, {comune}"'
            })
        
        raw_data = loc.raw
        location_type = raw_data.get('geometry', {}).get('location_type', 'UNKNOWN')
        address_components = raw_data.get('address_components', [])
        
        has_route = any('route' in comp.get('types', []) for comp in address_components)
        has_street_number = any('street_number' in comp.get('types', []) for comp in address_components)
        formatted_address = loc.address
        
        is_specific = location_type in ['ROOFTOP', 'RANGE_INTERPOLATED'] or (has_route and has_street_number)
        ...
        if is_specific:
            return (loc.latitude, loc.longitude, {
                ...
                'message': f'✅ Trovato: {formatted_address}'
            })
        elif has_route:
            return (loc.latitude, loc.longitude, {
                'success': True,
                'type': 'street',
                'display_name': formatted_address,
                'message': f'✅ Via trovata: {formatted_address}\n⚠️ N. civico approssimato'
            })
        else:
            return (0, 0, {
                'success': False,
                'type': 'city',
                'display_name': formatted_address,
                'message': f'⚠️ Trovato solo: {formatted_address}\n💡 Prova con indirizzo completo'
            })
